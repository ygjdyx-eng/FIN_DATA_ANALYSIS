...
FOR recday  IN  c_aging_idsday  LOOP   
    vt_sql:='update TEMP_FINANCE_AGING_ANALYSISPLIS05 set ZERO_CLOSING_MARKER=''2'' where id in (
SELECT ID FROM (
...
FOR rec  IN  c_aging_ids  LOOP
    ...
    --对可结零的数据下次不再展示 
	v_sql:='
    UPDATE TEMP_FINANCE_AGING_ANALYSISPLIS05 a SET ZERO_CLOSING_MARKER=''1''  WHERE a.ID IN (
	SELECT aa.id FROM 
	(SELECT  
