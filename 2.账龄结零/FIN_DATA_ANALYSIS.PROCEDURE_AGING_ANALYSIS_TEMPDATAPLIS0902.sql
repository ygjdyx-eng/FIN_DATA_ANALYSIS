CREATE OR REPLACE PROCEDURE FIN_DATA_ANALYSIS.PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS0902(executemonth IN varchar2) AS
...
BEGIN
    ...
    VALUES (sequence_course_method_log.nextval, executemonth, 'PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS0902', 'START', '【账龄临时数据生成】方法正在执行');

   
    -- 把当月金额<>0的数据放入临时账龄表中
    -- 个险数据插入
   -- 个险
INSERT INTO TEMP_FINANCE_AGING_ANALYSISPLIS09 (
    id,
    ledger_id,
    short_name,
    product_no,
...
SELECT 
    sequence_temp_FINANCE_Aging_analysisplis09.nextval AS id,
    tm1.LEDGER_ID,
    tm1.SHORT_NAME,
    tm1.PRODUCT_NO,
...
FROM temp_month_personal_insurance09 tm1
JOIN (
    SELECT 
        SHORT_NAME, 
        PERIOD_NAME, 
        ...
        PRODUCT_CODE_NEW,
        DEFAULT_EFFECTIVE_DATE
    FROM temp_month_personal_insurance09
    GROUP BY 
        SHORT_NAME, 
        PERIOD_NAME, 
        SEGMENT3, 
...
FOR recday  IN  c_aging_idsday  LOOP   
    ...
    vt_sql:='update TEMP_FINANCE_AGING_ANALYSISPLIS09 set ZERO_CLOSING_MARKER=''2'', SHORT_NAME=:close_month where ZERO_CLOSING_MARKER=''0'' AND HANDLE_STATUS=0 and id in (
SELECT ID FROM (
select POLICY_NO,SUBJECT_ID from TEMP_FINANCE_AGING_ANALYSISPLIS09 a 
...
(SELECT SUBJECT_ID,POLICY_NO,B.ID  FROM TEMP_FINANCE_AGING_ANALYSISPLIS09 B 
...
INSERT INTO temp_finally_FINANCE_AGING_ANALYSIS09 (
    id,
    LEDGER_ID,
    SHORT_NAME,
    PRODUCT_NO,
...
SELECT 
    ...
    '0' AS ZERO_CLOSING_MARKER
FROM TEMP_FINANCE_AGING_ANALYSISPLIS09 faa
...
JOIN (
    SELECT 
        ...
        JE_SOURCE,
        ACCOUNT_SEGMENT
    FROM TEMP_FINANCE_AGING_ANALYSISPLIS09
...
FOR rec  IN  c_aging_ids  LOOP
	...
	v_fristsql:='UPDATE temp_finally_FINANCE_AGING_ANALYSIS09 SET ZERO_CLOSING_MARKER=''1''  
    ...
    FROM temp_finally_FINANCE_AGING_ANALYSIS09 a 
	...
	(SELECT B.DISTRICT_ID,B.SUBJECT_ID ,B.POLICY_NO,B.ID  FROM temp_finally_FINANCE_AGING_ANALYSIS09 B
    ...
    --对可结零的数据下次不再展示 
	v_sql:='
    UPDATE TEMP_FINANCE_AGING_ANALYSISPLIS09 a SET ZERO_CLOSING_MARKER=''1'', SHORT_NAME=:close_month WHERE a.ZERO_CLOSING_MARKER=''0'' AND a.HANDLE_STATUS=0 AND a.ID IN (
	SELECT aa.id FROM 
	(SELECT  
    ...
    FROM TEMP_FINANCE_AGING_ANALYSISPLIS09 faa WHERE faa.ZERO_CLOSING_MARKER=''0'' AND faa.HANDLE_STATUS=0 ) aa 
    ...
    -- 执行结束记录日志  ce
    INSERT INTO course_method_log (id, EXECUTE_MONTH, METHOD_NAME, METHOD_STATUS, DESCRIPTION)
    VALUES (sequence_course_method_log.nextval, executemonth, 'PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS0902', 'END', '【账龄临时分析】方法执行结束');
