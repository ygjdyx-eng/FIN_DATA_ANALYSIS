...
FOR recday  IN  c_aging_idsday  LOOP   
    ...
    vt_sql:='update TEMP_FINANCE_AGING_ANALYSISPLIS09 set ZERO_CLOSING_MARKER=''2'', SHORT_NAME=:close_month where ZERO_CLOSING_MARKER=''0'' and id in (
SELECT ID FROM (
...
FOR rec  IN  c_aging_ids  LOOP
    ...
    --对可结零的数据下次不再展示 
	v_sql:='
    UPDATE TEMP_FINANCE_AGING_ANALYSISPLIS09 a SET ZERO_CLOSING_MARKER=''1'', SHORT_NAME=:close_month WHERE a.ZERO_CLOSING_MARKER=''0'' AND a.ID IN (
	SELECT aa.id FROM 
	(SELECT  
    ...
    FROM TEMP_FINANCE_AGING_ANALYSISPLIS09 faa) aa 
