CREATE OR REPLACE PROCEDURE FIN_DATA_ANALYSIS.PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS0102(executemonth IN varchar2) AS
    ...
    v_fristsql VARCHAR2(32767); -- 区间结零动作
...
BEGIN
    ...
    VALUES (sequence_course_method_log.nextval, executemonth, 'PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS0102', 'START', '【账龄临时数据生成】方法正在执行');
...
INSERT INTO TEMP_FINANCE_AGING_ANALYSISPLIS01 (
    id,
    ledger_id,
    short_name,
    product_no,
...
FROM temp_month_personal_insurance01 tm1
JOIN (
    SELECT 
        SHORT_NAME, 
        PERIOD_NAME, 
...
FOR recday  IN  c_aging_idsday  LOOP   
    ...
    vt_sql:='update TEMP_FINANCE_AGING_ANALYSISPLIS01 set ZERO_CLOSING_MARKER=''2'', SHORT_NAME=:close_month where ZERO_CLOSING_MARKER=''0'' AND HANDLE_STATUS=0 and id in (
      SELECT ID FROM (
		select POLICY_NO,SUBJECT_ID from TEMP_FINANCE_AGING_ANALYSISPLIS01 a 
		...
		(SELECT SUBJECT_ID,POLICY_NO,B.ID  FROM TEMP_FINANCE_AGING_ANALYSISPLIS01 B 
...
INSERT INTO temp_finally_FINANCE_AGING_ANALYSIS01 (
    id,
    LEDGER_ID,
    SHORT_NAME,
    PRODUCT_NO,
...
SELECT 
    ...
    '0' AS ZERO_CLOSING_MARKER
FROM TEMP_FINANCE_AGING_ANALYSISPLIS01 faa
...
--对于可结零的数据，下次不再展示，如同一 科目，保单，机构 的数据 账龄区间在1到3月和3到12月的期间金额相加可为0，则明细下次不参与计算,
FOR rec  IN  c_aging_ids  LOOP
	--结零
	v_fristsql:='UPDATE temp_finally_FINANCE_AGING_ANALYSIS01 SET ZERO_CLOSING_MARKER=''1''  
    ...
    FROM temp_finally_FINANCE_AGING_ANALYSIS01 a 
	...
	(SELECT B.DISTRICT_ID,B.SUBJECT_ID ,B.POLICY_NO,B.ID  FROM temp_finally_FINANCE_AGING_ANALYSIS01 B
    ...
    --对可结零的数据下次不再展示 
	v_sql:='
    UPDATE TEMP_FINANCE_AGING_ANALYSISPLIS01 a SET ZERO_CLOSING_MARKER=''1'', SHORT_NAME=:close_month WHERE a.ZERO_CLOSING_MARKER=''0'' AND a.HANDLE_STATUS=0 AND a.ID IN (
	SELECT aa.id FROM 
	(SELECT  
    ...
    FROM TEMP_FINANCE_AGING_ANALYSISPLIS01 faa WHERE faa.ZERO_CLOSING_MARKER=''0'' AND faa.HANDLE_STATUS=0 ) aa 
    ...
    -- 执行结束记录日志  ce
    INSERT INTO course_method_log (id, EXECUTE_MONTH, METHOD_NAME, METHOD_STATUS, DESCRIPTION)
    VALUES (sequence_course_method_log.nextval, executemonth, 'PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS0102', 'END', '【账龄临时数据生成】方法执行结束');
