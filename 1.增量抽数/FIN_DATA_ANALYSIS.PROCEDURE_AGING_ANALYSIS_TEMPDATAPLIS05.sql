CREATE OR REPLACE PROCEDURE FIN_DATA_ANALYSIS.PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS05(executemonth IN varchar2 )
AS
BEGIN
	INSERT INTO course_method_log (id,EXECUTE_MONTH,METHOD_NAME,METHOD_STATUS,DESCRIPTION) VALUES (sequence_course_method_log.nextval,executemonth,'PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS05','START','【财务账龄分析临时数据生成】方法正在执行');

		-- 情空上个月的桥梁表数据，
		EXECUTE  IMMEDIATE 'TRUNCATE TABLE temp_month_personal_insurance05';
		EXECUTE  IMMEDIATE 'TRUNCATE TABLE temp_finally_FINANCE_Aging_analysis05';
	
	-- 设置当前月份的桥梁表数据，添加个险数据
    INSERT INTO temp_month_personal_insurance05 (
        id,
        POLICY_NO,    -- 保单号
        JE_SOURCE ,   --来源
        SEGMENT3,     --科目
    ...
    SELECT
        sequence_temp_month_personal_insurance05.nextval AS id,
        f.POLICY_NO , -- 保单号
        'PLIS' JE_SOURCE, --来源
        f.SEGMENT3,       --科目
    ...
    WHERE f.LEDGER_ID != 2222
      ...
      AND f.SEGMENT1 IN (SELECT distinct SEGMENT1 FROM FRS_DIM_COA_COAGING a WHERE  BRANCH_CODE LIKE '001005%')
      AND f.LEDGER_ID <>'2030' -- 排除数据来源为 xx的数据
      AND f.JE_SOURCE IN ('PLIS','EFT','PLI','FCS')
      AND SEGMENT3 IN (SELECT a.SUBJECT FROM SUBJECTENUMERATIONAGING a WHERE a.JE_SOURCE ='PLIS')
	  and f.SHORT_NAME  not like 'THTF_I17%';
	  
	  INSERT INTO course_method_log (id,EXECUTE_MONTH,METHOD_NAME,METHOD_STATUS,DESCRIPTION) VALUES (sequence_course_method_log.nextval,executemonth,'PROCEDURE_AGING_ANALYSIS_TEMPDATAPLIS05','START','【财务账龄分析临时数据生成】方法执行完成');
